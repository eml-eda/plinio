...
from flexnas.methods import SuperNetModule, PaddedModule


def get_reference_model(model_name: str, model_config: Optional[Dict[str, Any]] = None
                        ) -> nn.Module:
    if model_name == 'resnet_8_sn':
        return ResNet8SN_Padded()
    else:
...
class ConvBlock(torch.nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size=3, stride=2, padding=1):
        ...
        self.conv1 = SuperNetModule([
            ...
            PaddedModule(nn.Identity(), out_channels-in_channels)
        ])
...
class ResNet8SN_Padded(torch.nn.Module):
    def __init__(self):
        super().__init__()
