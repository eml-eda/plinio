...
from ..quant.nn import QuantConv3d, QuantList
...
class MPSConv3d(nn.Conv3d, MPSModule):
    """A nn.Module implementing a Conv3d layer with mixed-precision search support

    :param conv: the inner `nn.Conv3d` layer to be optimized
    :type conv: nn.Conv3d
    ...
    def __init__(self,
        ...
        if isinstance(w_mps_quantizer, MPSPerChannelQtz) and (w_mps_quantizer.zero_index is not None) and \
                conv.groups != 1:
            msg = ('MPS currently supports only 3d Conv. with groups=1 when using 0-bit precision,'
    ...
    def forward(self, input: torch.Tensor) -> torch.Tensor:
        ...
        - Compute Conv3d operation using the previous obtained quantized tensors.
    ...
    def autoimport(n: fx.Node,
                   ...
                   w_mps_quantizer: Union[MPSPerLayerQtz, MPSPerChannelQtz],
                   b_mps_quantizer: MPSBiasQtz):
        """Create a new fx.Node relative to a MPSConv3d layer, starting from the fx.Node
        of a nn.Conv3d layer, and replace it into the parent fx.GraphModule

        :param n: a fx.Node corresponding to a nn.Conv3d layer
        :type n: fx.Node
        ...
        if type(submodule) != nn.Conv3d:
            msg = f"Trying to generate MPSConv3d from layer of type {type(submodule)}"
            raise TypeError(msg)
        submodule = cast(nn.Conv3d, submodule)
        new_submodule = MPSConv3d(submodule,
                                  out_mps_quantizer,
                                  w_mps_quantizer,
                                  b_mps_quantizer)
    ...
    def export(n: fx.Node, mod: fx.GraphModule):
        """Replaces a fx.Node corresponding to a MPSConv3d layer,
        with the selected fake-quantized nn.Conv3d layer within a fx.GraphModule
        ...
        if isinstance(submodule.w_mps_quantizer, MPSPerLayerQtz):
            if submodule.bias is not None:
                ...
                b_quantizer = None
            new_submodule = QuantConv3d(submodule,
        ...
        # per-channel search => multiple precision/quantizers
            ...
            for prec, w_quant in prec_and_quantz.items():
                ...
                if out_channels == 0:  # no out_channels for the current prec
                    continue
                new_conv = nn.Conv3d(submodule.in_channels,
                                     out_channels,
                ...
                with torch.no_grad():
                    ...
                    if submodule.bias is not None:
                        ...
                        b_quantizer = None
                quant_conv = QuantConv3d(new_conv,
                                         submodule.selected_in_quantizer,
                                         submodule.selected_out_quantizer,
                                         w_quant,
                                         b_quantizer)
