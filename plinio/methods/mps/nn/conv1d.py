...
from ..quant.nn import QuantConv1d, QuantList
...
# TODO: ugly duplication of MPSConv2d, refactor


class MPSConv1d(nn.Conv1d, MPSModule):
    """A nn.Module implementing a Conv1d layer with mixed-precision search support
    ...
    def forward(self, input: torch.Tensor) -> torch.Tensor:
        ...
        - Compute Conv1d operation using the previous obtained quantized tensors.
    ...
    def autoimport(n: fx.Node,
                   ...
                   w_mps_quantizer: Union[MPSPerLayerQtz, MPSPerChannelQtz],
                   b_mps_quantizer: MPSBiasQtz):
        """Create a new fx.Node relative to a MPSConv1d layer, starting from the fx.Node
        of a nn.Conv1d layer, and replace it into the parent fx.GraphModule

        :param n: a fx.Node corresponding to a nn.Conv1d layer
        :type n: fx.Node
        ...
        if type(submodule) != nn.Conv1d:
            msg = f"Trying to generate MPSConv1d from layer of type {type(submodule)}"
            raise TypeError(msg)
        submodule = cast(nn.Conv1d, submodule)
        new_submodule = MPSConv1d(submodule,
                                  out_mps_quantizer,
                                  w_mps_quantizer,
                                  b_mps_quantizer)
    ...
    def export(n: fx.Node, mod: fx.GraphModule):
        """Replaces a fx.Node corresponding to a MPSConv1d layer,
        with the selected fake-quantized nn.Conv1d layer within a fx.GraphModule
        ...
        if isinstance(submodule.w_mps_quantizer, MPSPerLayerQtz):
            if submodule.bias is not None:
                ...
                b_quantizer = None
            new_submodule = QuantConv1d(submodule,
        ...
        # per-channel search => multiple precision/quantizers
            ...
            for prec, w_quant in prec_and_quantz.items():
                ...
                if out_channels == 0:  # no out_channels for the current prec
                    continue
                new_conv = nn.Conv1d(submodule.in_channels,
                                     out_channels,
                ...
                with torch.no_grad():
                    ...
                    if submodule.bias is not None:
                        ...
                        b_quantizer = None
                quant_conv = QuantConv1d(new_conv,
                                         submodule.selected_in_quantizer,
                                         submodule.selected_out_quantizer,
                                         w_quant,
                                         b_quantizer)
